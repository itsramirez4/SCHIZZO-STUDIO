...
export const DOC_PAGES: DocPage[] = [
  ...
  },
  {
    ...
    content: [
      ...
      'Rueda cromática y armonías: el ángulo de la rueda es el tono y la distancia al centro, la saturación; una barra ajusta el brillo. La armonía elegida (complementaria, análoga, tríada, tetrada…) se dibuja sobre la rueda como puntos unidos: arrastra el color base y toda la armonía gira con él; pulsa un punto para usar ese color (Mayús: como secundario). Hay dos ruedas: la de pintor (RYB), donde rojo–verde, azul–naranja y amarillo–violeta son complementarios y la tríada es rojo–amarillo–azul, y la de pantalla (RGB), donde el rojo se opone al cian. En la caja de herramientas, el selector avanzado también se puede cambiar entre el cuadrado clásico y la rueda.',
  ...
  },
  {
    ...
    content: [
      ...
      'Los pinceles admiten sensibilidad a la presión, inclinación del lápiz (ancho de la punta), giro según la dirección del trazo y estabilizador (suavizado). Puedes importar varias imágenes PNG/JPG a la vez como puntas de pincel. Desde el editor de pinceles se importan librerías .abr de Photoshop (todas las versiones), pinceles .brush de Procreate y presets .kpp de Krita: puntas, tamaño, espaciado, dispersión y dinámicas de presión. En el .abr moderno se importan también la redondez y el ángulo de la punta (exactos), y la textura de papel, el pincel dual, el ruido y los bordes húmedos de forma aproximada: los tres primeros se integran en la punta y los bordes húmedos se aplican al trazo. También se importa la dinámica de color (mezcla de primer plano y fondo, tono, saturación, brillo y pureza). Procreate (.brush) y los paquetes de Krita (.bundle, con sus puntas .gbr/.gih/.png) se han verificado con archivos reales; se importan tamaño, espaciado, presión, orientación y dispersión, y el grano de Procreate se integra en la punta. En Krita se importan además las curvas de sensibilidad, los pinceles animados (.gih, con todos sus fotogramas, al azar o en orden), los borradores y los modos de fusión (que se aplican una vez por trazo, como en Krita). Los motores que no pintan también se importan como pinceles que ejecutan la herramienta equivalente: difuminar (mezclador de color), deformar (licuar) y clonar (Alt+clic fija el origen y se pinta con lo que hay allí; con «healing» se iguala el brillo de la zona). El motor «experiment» se importa como relleno del contorno del trazo. Las texturas de Krita (vienen dentro de cada preset) se integran en la punta, y el modo «embotar» del difuminado pinta con el color medio de la zona. Un .kpp suelto necesita que selecciones a la vez sus ficheros de punta (.gbr/.gih/.png). Los .abr antiguos (v1/v2) importan tanto las puntas de imagen como los pinceles redondos (con su dureza, redondez y ángulo); su lectura coincide con la de GIMP y con la documentación de AbrKit, pero sigue sin probarse con un archivo real.',
    ],
  },
  {
    ...
    content: [
      ...
      'Perspectiva → Regla: mientras hay una regla activa, los trazos a mano alzada (pincel, borrador y difuminado) se pegan a ella, y salen limpios aunque la mano tiemble. «Regla recta»: un trazo que empieza cerca de la regla la sigue (arrastra el centro para moverla y el extremo para girarla; Mayús gira de 15° en 15°). «Paralelas»: todos los trazos toman el ángulo de la regla, empiecen donde empiecen. «Elipse / círculo»: un trazo que empieza cerca del contorno lo sigue; se mueve, se gira y se ajustan sus dos radios. «Perspectiva»: el trazo sigue la línea desde donde empieza hasta un punto de fuga de la grilla (o la vertical): mueve el lápiz hacia donde quieras la línea. Se combina con la simetría y se guarda con el proyecto. Con el tirador de un extremo bajo el lápiz se arrastra la regla en lugar de dibujar; los de la elipse quedan justo fuera del contorno para no estorbar.',
  ...
  },
  {
    ...
    content: [
      ...
      'Capas vectoriales (icono de formas en el panel de Capas): las formas, el texto vectorial, el texto normal (con sus efectos), el texto en trazo y los trazados de pluma que dibujes sobre ellas quedan como objetos editables. Con «Seleccionar objeto vectorial» (A) los mueves; en la lista de la capa cambias texto, tamaño, colores, trazo, posición, giro y orden, o los eliminas. Para pintar píxeles sobre una capa vectorial hay que rasterizarla antes.',
  ...
  },
  {
    ...
    content: [
      ...
      'Reproducir el proceso: en el panel Historial (y en Grabación de sesión), «Reproducir el proceso» abre un reproductor que recorre el dibujo paso a paso: cada trazo, relleno, filtro o cambio de capa es un paso. Puedes avanzar y retroceder de uno en uno (flechas), arrastrar la barra, reproducir a 1–40 pasos por segundo (Espacio) y exportar el resultado como timelapse WebM. Los pasos se guardan en este equipo junto con el proyecto, así que la reproducción sigue disponible al cerrar y volver a abrirlo; en sesiones muy largas se conservan hasta 600 pasos (se aclaran los intermedios, nunca el primero ni el último). No hace falta activar nada, y no sustituye a la grabación en vídeo, que sí captura el tiempo real.',
  ...
  },
  {
    ...
    content: [
      ...
      'Las medidas son del plano de la imagen: un brazo hacia la cámara o una figura de perfil dan valores engañosos. La colocación automática de puntos usa un modelo de pose (se descarga una vez, 12 MB, y queda en tu equipo) pensado para fotografías. Con dibujos de línea, la app recorta la figura y prueba varias versiones más parecidas a una foto (contorno relleno y sombreado, líneas engrosadas) y se queda con la lectura más segura; si esa lectura es dudosa te lo avisa y los puntos son solo un punto de partida. Si el dibujo es un retrato o un busto (sin caderas ni piernas) no hay figura que medir, pero se busca la cabeza: aparece la tarjeta «Rostro encontrado» con la inclinación de la cabeza, un botón para medir la línea de los ojos y otro para poner una guía de rostro ya alineada. Con retratos reales funciona en aproximadamente la mitad de los casos (falla con gafas de sol, ojos cerrados o la cabeza muy girada); los bocetos muy simples, las figuras incompletas o las poses raras pueden seguir fallando. En un adulto frontal la altura sale con ~3-4 % de error; en niños no es fiable. Si falla, coloca los puntos a mano.',
  ...
  },
  {
    ...
    content: [
      ...
      'Qué hace, y con qué medios: pose y expresión a partir de una descripción («mujer corriendo con los brazos hacia arriba», «muy sorprendido») que colocan el maniquí 3D. Es un intérprete de palabras clave con tolerancia a erratas (corrige «corrindo» → «corriendo» y te lo dice) que también entiende inglés, sinónimos («a toda pastilla», «en jarras»), formas verbales («corre», «se sienta») y negaciones de cuerpo («sin cruzar los brazos»). Te indica qué entendió y qué ignoró; no entiende frases arbitrarias. Para frases libres, si configuras una API compatible con OpenAI aparece «Entender frase libre»: se le envía solo tu frase para que la reescriba con las palabras que el intérprete conoce, y el intérprete sigue siendo quien la traduce a articulaciones. Si no se leen los ojos (gafas, ojos cerrados, cabeza girada) la cabeza se coloca por las proporciones de los hombros y se marca como estimación para que ajustes la guía a mano; en 5 de 7 dibujos de prueba se encontró la cabeza, y falla cuando tampoco se leen los hombros. Convertir la pose de tu dibujo en el maniquí 3D: busca la figura (o usa tus puntos) y ajusta las articulaciones a la vista frontal. Un dibujo plano no dice si un brazo o una pierna acortados apuntan hacia ti o hacia atrás, así que puedes elegirlo por extremidad y reajustar; la inclinación del torso hacia la cámara no se deduce. Variaciones de iluminación: leen el brillo como relieve, iluminan desde otra dirección y proyectan las sombras de los relieves sobre lo que hay detrás. En un dibujo de línea, sin sombreado que leer, «inflan» los volúmenes a partir de las líneas (cada forma se abomba hacia su centro): es un volumen inventado y suave para estudiar la luz, no algo que el dibujo contenga. Nunca es una simulación física. Análisis de anatomía y proporciones; paletas desde un texto o desde tus colores; consejos de composición con mapa de atención; limpieza de motas y huecos con vista previa (por defecto solo quita polvo aislado y compacto: conserva los trozos alargados y los puntos junto a una línea, que suelen ser trazo); separación en capas (línea/color, sujeto/fondo, por colores; la capa de color rellena bajo la línea, también sobre fondos no blancos). Todo eso se calcula en tu equipo.',
      'Generar referencias con imágenes: opcional y solo si configuras un proveedor (un servidor local de Stable Diffusion o una API compatible con OpenAI). Ves y puedes editar el texto exacto antes de enviarlo; solo se envía al pulsar «Generar». La clave de API se guarda cifrada en tu equipo, solo la usa el proceso principal, y las direcciones remotas deben ser https. «Probar conexión» comprueba la dirección y lista los modelos disponibles. El resultado se guarda en tu biblioteca de referencias con la etiqueta «ia» y, si quieres, como capa de referencia bloqueada que no se exporta. La calidad depende del proveedor que elijas, y la conexión está probada con servidores simulados, no con un servicio real.',
    ],
  },
  {
    id: 'left-handed',
