// @ts-nocheck
/*
 * Vendored from ag-psd 31.0.2 (dist/abr.js) — MIT License, Copyright (c) 2016 Agamnentzar.
 * https://github.com/Agamnentzar/ag-psd
 * Changes: converted to ES module imports; two PATCH comments below make it tolerate real-world files
 * that the original rejects entirely (a missing dynamics descriptor aborted the whole file).
 */
import { BlnM, parseAngle, parsePercent, parseUnitsToNumber, readVersionAndDescriptor } from 'ag-psd/dist/descriptor';
import { checkSignature, createReader, readBytes, readDataRLE, readInt16, readInt32, readPascalString, readPattern, readSignature, readUint16, readUint32, readUint8, skipBytes } from 'ag-psd/dist/psdReader';
...
function parseDynamics(desc) {
    // PATCH (SCHIZZO): some real files list a dynamics group without its descriptor.
            ...
            switch (type) {
                ...
                case 'desc': {
                    ...
                    for (const brush of desc.Brsh) {
                        // PATCH (SCHIZZO): convert each brush independently.
                        try {
                        const b = {
                            name: brush['Nm  '],
                        ...
                        brushes.push(b);
                            ...
                            try { brushes.push({ name: brush['Nm  '], shape: parseBrushShape(brush.Brsh), spacing: parsePercent(brush.Spcn) }); } catch (e2) { /* unreadable brush: skipped */ }
                        }
                    }
                    break;
                }
