...
export default function ReferenceWindowView() {
  ...
  if (!data) {
    return <div className="h-screen flex items-center justify-center bg-panel text-textDim text-xs">{t('referenceWindow.loading')}</div>;
  }
  ...
  return (
    <div className="h-screen flex flex-col bg-panel">
      <div className="flex flex-wrap items-center gap-1.5 p-1.5 bg-panelLight border-b border-border" data-testid="ref-window-toolbar">
        <button onClick={() => setFlipH((v) => !v)} className={btnClass(flipH)} title={t('referenceWindow.flipHTitle')}>
          Flip H
        </button>
        <button onClick={() => setFlipV((v) => !v)} className={btnClass(flipV)} title={t('referenceWindow.flipVTitle')}>
          Flip V
        </button>
        <span className="w-px h-4 bg-border" />
        <button onClick={() => zoomBy(1 / 1.25)} className={btnClass(false)} title={t('referenceWindow.zoomOut')}>
          −
        </button>
        <span className="text-[11px] text-textDim w-10 text-center" data-testid="ref-window-zoom">
          {Math.round(zoom * 100)}%
        </span>
        <button onClick={() => zoomBy(1.25)} className={btnClass(false)} title={t('referenceWindow.zoomIn')}>
          +
        </button>
        <span className="w-px h-4 bg-border" />
        <button onClick={() => rotateBy(-15)} className={btnClass(false)} title={t('referenceWindow.rotateLeft')}>
          ↺
        </button>
        <input type="range" min={-180} max={180} value={rotation} onChange={(e) => setRotation(Number(e.target.value))} className="w-24" title={t('referenceWindow.angle')} />
        <button onClick={() => rotateBy(15)} className={btnClass(false)} title={t('referenceWindow.rotateRight')}>
          ↻
        </button>
        ...
        <span className="w-px h-4 bg-border" />
        <button onClick={resetView} className={btnClass(false)} title={t('referenceWindow.fitTitle')}>
          {t('referenceWindow.fit')}
        </button>
        <button onClick={toggleAlwaysOnTop} className={btnClass(alwaysOnTop)}>
          {t('referenceWindow.alwaysOnTop')}
        </button>
      </div>
      <div
