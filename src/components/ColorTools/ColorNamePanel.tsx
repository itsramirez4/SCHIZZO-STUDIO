...
export default function ColorNamePanel() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <div className="text-[9px] text-textDim font-mono">
            {primaryColor} · más cercano: {best.hex} (ΔE {best.distance.toFixed(1)})
          </div>
        </div>
      </div>

      <div>
        <div className="text-[10px] text-textDim uppercase tracking-wide mb-1">Otros nombres cercanos</div>
        <div className="grid grid-cols-2 gap-1.5">
          {matches.slice(1).map((m) => (
            <button
              ...
              title={`Usar ${m.hex} como color primario`}
            >
      ...
      <div>
        <div className="text-[10px] text-textDim uppercase tracking-wide mb-1">Buscar por nombre</div>
        <input
          type="text"
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="ej. azul, coral, oliva…"
        ...
        {search.trim() && (
          <div className="grid grid-cols-2 gap-1.5 mt-1.5 max-h-40 overflow-y-auto">
            {searchResults.length === 0 ? (
              <p className="text-[10px] text-textDim col-span-2">Sin resultados.</p>
            ) : (
              searchResults.map((m) => (
                <button
                  key={`${m.hex}-${m.name}`}
